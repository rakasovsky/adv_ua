...
jQuery(document).ready(function ($) {
  ...
  $('.dropbtn').on({
    'click': function click() {
      ...
      $('.header_wrapper').css('background-color', 'rgba(23,23,23,1)'); // $(this).toggleClass('arrowDown arrowUp');
    }
  });
  ...
  $('.menu_tel').on({
    'click': function click() {
      $('.phones').toggle(100).css('visibility', 'visible'); // $('.tel_t').toggle();
    }
  ...
  $(".adv_sm_btn").click(function () {
    var $button = $(this);
    var $cont = $button.siblings('.main_gallery');
  ...
  $(".s_m").click(function () {
    var $button1 = $(this);
    var $cont1 = $button1.siblings('.more_posts');
    $cont1.toggleClass('visible_g').toggleClass('hidden_g'); // $button.text($button.text() === "Показать больше" ? "Закрыть": "Показать больше");
  }); //Vacancy buttons

  $('.t_btn').click(function () {
    var $vbtn = $(this);
    var $vcont = $vbtn.siblings('.vacancy_item');
  ...
  var generateHtml = function generateHtml(links) {
    var keys = Object.keys(links);
    var ali = keys.map(function (link) {
      var title = links[link];
  ...
  $('.menu_category_bnt2').on({
    'click': function click() {
      $('.change-image').attr('src', '/src/img/cat/min/odezjda-min.jpg'); // addClass('opacity_1') ???
  ...
  $('.menu_category_bnt11').on({
    'click': function click() {
      ...
      $('.menu_subcategory').html('<ul><li><a href="/eksklyuziv/izgotovlenie-mebeli-iz-metalla-i-dereva">Изготовление мебели из металла и дерева</a></li><li><a href="/eksklyuziv/chasy-i-predmety-interera">Годинники та предмети інтер\'єру</a></li><li><a href="/eksklyuziv/elementy-osveshcheniya">Елементи освітлення</a></li><li><a href="/eksklyuziv/nestandartnye-reklamnye-konstrukcii">Нестандартні рекламні конструкції</a></li><li><a href="/eksklyuziv/cvetodinamicheskie-displei">Кольороводинамічні дисплеі</a></li></ul>');
    }
  }); // Дополнительные услуги
  ...
  targets.forEach(lazyload); // Rotate on scroll
...
var instaLink = document.querySelector('.insta_link'),
    ...
    copyright = document.querySelectorAll('.copyright'); // Меню
...
linkThird.textContent = 'Контакти'; // Социалки

instaLink.href = 'https://www.instagram.com/advance_branding/';
linkedLink.href = 'https://www.linkedin.com/company/advc-branding';
faceLink.href = 'https://www.facebook.com/advc.com.ua'; // Адрес

fStreet.textContent = 'переулок Разъездной, 27'; // Карта

googleMap.src = 'https://www.google.com/maps/embed?pb=!1m18!1m12!1m3!1d10264.725756149272!2d36.23118855539478!3d49.97045585414487!2m3!1f0!2f0!3f0!3m2!1i1024!2i768!4f13.1!3m3!1m2!1s0x4127a07c5d566615%3A0x8af373bf79739dc4!2z0L_RgNC-0LLRg9C70L7QuiDQoNC-0Lcn0ZfQt9C90LjQuSwgMjcsINCl0LDRgNC60ZbQsiwg0KXQsNGA0LrRltCy0YHRjNC60LAg0L7QsdC70LDRgdGC0YwsIDYxMDAw!5e0!3m2!1suk!2sua!4v1622799369734!5m2!1suk!2sua'; // Копирайт

var newCopyright = new Date();
var newCopyYear = newCopyright.getFullYear();
copyright.forEach(function (i) {
  i.textContent = "".concat(newCopyYear);
});